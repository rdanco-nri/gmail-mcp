...
export const toolDefinitions: ToolDefinition[] = [
  ...
  // =====================================================================
  {
    ...
    description: [
      ...
      "",
      "FORMATTING: handled automatically, no flag needed. Sets explicit text wrapping on the written range, auto-fits row heights to the new content, and — if the new data has more columns than the tab previously had — extends the last existing header cell's style and column width onto the new column(s). The plain values API never touches formatting on its own, which is why this exists; if it ever fails, the response carries a `formatWarning` but the values write still succeeds.",
      "",
      "SIDE EFFECTS: clears and rewrites the entire named tab; formatting/sizing changes per above. Not reversible through this MCP — recover via Google Sheets' built-in Version History (File > Version history) if needed. Requires the `spreadsheets` scope (full read-write; `spreadsheets.readonly` is rejected).",
    ].join("\n"),
