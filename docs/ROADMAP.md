...
- ✅ **Drive upload with convert-on-upload (v0.36)** — `drive_upload_file`: `files.create` with multipart media, converting `.pptx`/`.docx`/`.xlsx`/`.csv` to the Google-native type by default so a locally built deck lands in Slides with its theme; `replaceFileId` overwrites in place. Source path jailed to the attachment or download dir; 5 MB cap. First Drive-family tool with a registrar test (`src/tools/drive.test.ts`). Follow-up: resumable upload for files over 5 MB.
