...
- **`drive_upload_file`** — a deck built locally on the Newton template had no way into Google Slides except a hand upload, because the MCP had no upload call at all (`files.create` with media was unused). The new tool uploads a local file with `files.create` (multipart media) and, by default, converts it on the way in: `.pptx` → Slides, `.docx` → Docs, `.xlsx`/`.csv` → Sheets, so the theme and layouts inside the file survive. `replaceFileId` runs `files.update` with media instead, keeping the file's ID, URL, and sharing; whether Drive re-converts an Office file onto an existing native file is left to the result's `mimeType`. The source path is jailed to `GMAIL_MCP_ATTACHMENT_DIR` or `GMAIL_MCP_DOWNLOAD_DIR` through the new exported `assertReadablePathInJail` (the attachment-only check `send_email` uses is now a one-jail call to it), and files over 5 MB are refused before any API call (multipart cap; resumable upload is a follow-up). `scopes: ["drive"]` only — not `drive.file`, since `hasScope` is any-of and a `drive.file` token would 403 on a `replaceFileId` the app did not create. Wired into `WRITE_TOOLS` (dry-run) and the `workspace_writes` rate-limit bucket. `src/tools/drive.test.ts` is the first registrar test for the Drive family: 17 cases over the real MCP round-trip cover the jail (both roots, symlink escape, relative and missing paths), the extension map, the size cap, create vs. replace request shapes, the scope filter, 403/404/500 mapping, and the dry-run short-circuit.
