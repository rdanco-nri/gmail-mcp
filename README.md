...
# Add Sheets write (v0.33) on top of an existing v0.31/v0.32 grant.
# `spreadsheets` is a superset of `spreadsheets.readonly`, so it
# replaces (not adds to) that entry below. IMPORTANT: list every
# scope you want to keep — credentials.json's stored `scopes` array
# is overwritten wholesale by whatever you pass here, not merged. The
# actual Google consent IS incremental (`include_granted_scopes:
# true` + `prompt: "consent"`, both auto-set), so re-running this
# does not revoke anything and the browser typically only prompts for
# the new Sheets-write permission — but the locally-cached scope list
# that gates which tools this server advertises is fully replaced,
...
| Dry-run | `GMAIL_MCP_DRY_RUN=true` | unset (real calls) | When `"true"` (strict match), every write tool short-circuits before reaching Google and returns the redacted payload it would have sent. Covers Gmail writes (`send_email`, `reply_all`, `reply_to_email`, `forward_email`, `draft_email`, `update_draft`, `delete_draft`, `send_draft`, `delete_email`, `modify_email`, `batch_modify_emails`, `batch_delete_emails`, `create_label`, `update_label`, `delete_label`, `get_or_create_label`, `create_filter`, `delete_filter`, `create_filter_from_template`, `modify_thread`) and Drive/Slides/Docs/Sheets writes (`drive_reply_to_comment`, `drive_trash_file`, `drive_upload_file`, `slides_create_deck_from_outline`, `slides_append_to_deck`, `docs_create_release_doc`, `docs_write_tab`, `sheets_write_tab`). Useful for CI smoke tests, agent debugging, and human-in-the-loop approval flows. Read tools ignore the flag (nothing to preview). Matches `MERCURY_MCP_DRY_RUN` / `FAXDROP_MCP_DRY_RUN` on the sibling servers. |
...
- **Drive (v0.31)** — `drive_search`, `drive_get_metadata`, `drive_read_file`, `drive_download_file`, `drive_list_shared_drives`, `drive_list_comments`, `drive_reply_to_comment`, `drive_trash_file`. `drive_read_file` dispatches on mimeType: Docs → markdown via `files.export`, Sheets → all-tabs CSV via the Sheets API (Drive's `files.export(text/csv)` only returns the first tab), Slides → structured outline via the Slides API, PDFs/images/binaries → saved into the existing download jail. Comment discovery is via Gmail (`search_emails` for `from:comments-noreply@docs.google.com`); there's no Drive API "comment inbox" endpoint. `drive_upload_file` (v0.36) uploads a local file and, by default, converts it on the way in (`.pptx` → Slides, `.docx` → Docs, `.xlsx`/`.csv` → Sheets) so a deck built locally on a template lands in Slides with its theme and layouts intact; `replaceFileId` overwrites an existing file's content in place. The source path must sit inside `GMAIL_MCP_ATTACHMENT_DIR` or `GMAIL_MCP_DOWNLOAD_DIR` (refused before any API call otherwise), and files over 5 MB are refused (multipart cap; no resumable upload yet).
